...
public class Elevator1 extends TimerTask {
		static public myQueue<Integer> ele1=new myQueue<Integer>();
	   ...
	   public void run() {
		if(ele1.size==0) {
			while((count_inside+Control.floor0.peek())<11 & Control.floor0.size>0 & floor==0){
				  ele1.enqueue(Control.floor0.peek(),Control.floor0.peek1());
			...
			if(Control.floor1.size!=0 & floor==1) {
				while((count_inside+Control.floor1.peek())<11 & Control.floor1.size>0){
					  ele1.enqueue(Control.floor1.peek(),0);
					  target=0;
			...
			if(Control.floor2.size!=0 & floor==2) {
				while((count_inside+Control.floor2.peek())<11 & Control.floor2.size>0){
					  ele1.enqueue(Control.floor2.peek(),0);
					  target=0;
			...
			if(Control.floor3.size!=0 & floor==3) {
				while((count_inside+Control.floor3.peek())<11 & Control.floor3.size>0){
					  ele1.enqueue(Control.floor3.peek(),0);
					  target=0;
			...
			if(Control.floor4.size!=0 & floor==4) {
				while((count_inside+Control.floor4.peek())<11 & Control.floor4.size>0){
					  ele1.enqueue(Control.floor4.peek(),0);
					  target=0;
		...
		screen.count1.setText("count_inside:"+Integer.toString(count_inside));
		screen.inside1.setText("inside:"+ele1.q());
		screen.destination1.setText("destination:"+Integer.toString(target));
		  if(floor<target) {
			  floor++;
			  screen.floor_1.setText("floor:"+Integer.toString(floor));
			  screen.direction1.setText("direction:up");
		  }
		  if(floor>target) {
			  floor--;
			  screen.floor_1.setText("floor:"+Integer.toString(floor));
			  screen.direction1.setText("direction:down");
		  }
		  if(floor==target){
			  if(direction==1) {
				  if(floor==0) {
				  ...
				  if(floor==1) {
					  screen.floor1+=ele1.peek();
					  count_inside-=ele1.peek();
					  }
				  else if(floor==2) {
					  screen.floor2+=ele1.peek();
					  count_inside-=ele1.peek();
					  }
				  else if(floor==3) {
					  screen.floor3+=ele1.peek();
					  count_inside-=ele1.peek();
					   }
				  else if(floor==4) {
					  screen.floor4+=ele1.peek();
					  count_inside-=ele1.peek();
					  }
			  }
			  if(direction==0) {
				  if(floor==0 & ele1.size!=0) {
					  count_inside-=ele1.peek();
					  screen.exitcount+=ele1.peek();
				  }
			  }
			  screen.inside1.setText("inside:"+ele1.q());
			  screen.count1.setText("count_inside:"+Integer.toString(count_inside));
