...
public class Elevator2 extends TimerTask {
	static public myQueue<Integer> ele2=new myQueue<Integer>();
	   ...
	   public void run() {
		   if(ele2.size==0) {
				 while((count_inside+Control.floor0.peek())<11 & Control.floor0.size>0 & floor==0){
					  ele2.enqueue(Control.floor0.peek(),Control.floor0.peek1());
				...
				if(Control.floor1.size!=0 & floor==1) {
					while((count_inside+Control.floor1.peek())<11 & Control.floor1.size>0){
						  ele2.enqueue(Control.floor1.peek(),0);
						  target=0;
				...
				if(Control.floor2.size!=0 & floor==2) {
					while((count_inside+Control.floor2.peek())<11 & Control.floor2.size>0){
						  ele2.enqueue(Control.floor2.peek(),0);
						  target=0;
				...
				if(Control.floor3.size!=0 & floor==3) {
					while((count_inside+Control.floor3.peek())<11 & Control.floor3.size>0){
						  ele2.enqueue(Control.floor3.peek(),0);
						  target=0;
				...
				if(Control.floor4.size!=0 & floor==4) {
					while((count_inside+Control.floor4.peek())<11 & Control.floor4.size>0){
						  ele2.enqueue(Control.floor4.peek(),0);
						  target=0;
			...
			screen.count2.setText("count_inside:"+Integer.toString(count_inside));
			screen.inside2.setText("inside:"+ele2.q());
			screen.destination2.setText("destination:"+Integer.toString(target));
			  if(floor<target) {
				  floor++;
				  screen.floor_2.setText("floor:"+Integer.toString(floor));
				  screen.direction2.setText("direction:up");
			  }
			  if(floor>target) {
				  floor--;
				  screen.floor_2.setText("floor:"+Integer.toString(floor));
				  screen.direction2.setText("direction:down");
			  }
			  if(floor==target){
				  if(direction==1) {
					  if(floor==0) {
					  ...
					  if(floor==1) {
						  screen.floor1+=ele2.peek();
						  count_inside-=ele2.peek();
						  }
					  else if(floor==2) {
						  screen.floor2+=ele2.peek();
						  count_inside-=ele2.peek();
						   }
					  else if(floor==3) {
						  screen.floor3+=ele2.peek();
						  count_inside-=ele2.peek();
						  }
					  else if(floor==4) {
						  screen.floor4+=ele2.peek();
						  count_inside-=ele2.peek();
						  }
				  }
				  if(direction==0) {
					  if(floor==0 & ele2.size!=0) {
						  count_inside-=ele2.peek();
						  screen.exitcount+=ele2.peek();
					  }
				  }
				  screen.inside2.setText("inside:"+ele2.q());
				  screen.count2.setText("count_inside:"+Integer.toString(count_inside));
